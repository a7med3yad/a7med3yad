...
Codifying cloud infrastructure in reusable **Terraform modules** with remote state management. Delivered 20+ AWS resources as code, cutting environment provisioning time by 60% and eliminating manual drift.

### DevOps & CI/CD Automation
Building **GitHub Actions** pipelines for multi-environment deployments — no manual release steps. Containerized applications with **Docker**, shipped zero-downtime releases, and automated full build-test-deploy flows.
...
- **Backend:** 15+ endpoint ASP.NET Core Web API under Clean Architecture; JWT + OAuth2 (Google/GitHub)
- **Security:** AES-256 encryption on all submissions
- **Infrastructure:** Full AWS stack (VPC, EC2, IAM, S3) across 15+ Terraform resources
- **Frontend:** React SPA deployed to S3 static hosting
- **DevOps:** Dockerized services; zero-downtime releases via GitHub Actions CI/CD

`C#` `ASP.NET Core` `React` `AWS` `Terraform` `Docker` `GitHub Actions` `AES-256` `OAuth2`
